<script setup name="Product">
    ...
    import { getCategoryBrandAll } from "@/api/product/categoryBrand.js"
    import { getBrandAll } from "@/api/product/brand.js"
    import { getUnitAll } from "@/api/base/productUnit.js"
    import { getCategorySpecAll } from "@/api/product/productSpec.js"
    import { getToken } from "@/utils/auth.js"
    import {listProduct,getProduct,addProduct,updateProduct,delProduct,updateAuditStatus,updateStatus} from "@/api/product/product.js"
    /*------------------------ const ------------------------*/
...
<template>
    <div class="app-container">
        <!-- form表单 -->
        <el-form :inline="true" ref="queryRef" v-show="showSearch" label-width="68px" :model="queryParams">
            <el-form-item label="商品名称" prop="name">
                <el-input placeholder="请输入商品名称/" clearable v-model="queryParams.name" @keyup.enter="handleQuery"></el-input>
            </el-form-item>
            <el-form-item label="品牌">
                <el-select placeholder="选择品牌" v-model="queryParams.brandId" class="m-2" size="small" style="width: 100%">
                    <el-option v-for="item in queryBrandList" :key="item.id" :label="item.name" :value="item.id"/>
                </el-select>
            </el-form-item>
            <el-form-item label="分类">
                <el-select placeholder="一级分类" v-model="queryParams.category1Id" @change="selectcategory1" class="m-2" size="small" style="width: 32%;margin-right: 5px;">
                    <el-option v-for="item in queryCategory1List" :key="item.id" :label="item.name" :value="item.id"/>
                </el-select>
                <el-select placeholder="二级分类" v-model="queryParams.category2Id" @change="selectcategory2" class="m-2" size="small" style="width: 32%;margin-right: 5px;">
                    <el-option v-for="item in queryCategory2List" :key="item.id" :label="item.name" :value="item.id"/>
                </el-select>
                <el-select placeholder="三级分类" v-model="queryParams.category3Id" class="m-2" size="small" style="width: 32%">
        ...
        <!-- table表单 -->
        <el-table v-loadng="loading" :data="productList" @selection-change="handleSelectionChange">
            ...
            <el-table-column prop="category1Name" label="一级分类"></el-table-column>
            <el-table-column prop="category2Name" label="二级分类"></el-table-column>
            <el-table-column prop="category3Name" label="三级分类"></el-table-column>
            ...
            <el-form ref="productRef" :model="form" :rules="rules" label-width="80px" style="margin-top: 20px;">
                ...
                <div v-if="activeIndex == 0">
                    ...
                    <el-form-item label="轮播图" prop="sliderUrlList">
                        <el-upload v-model:file-list="sliderTempUrlList" :action="imgUpload.url" :headers="imgUpload.headers" list-type="picture-card" multiple :on-success="handleSliderSuccess" :on-remove="handleRemove">
                    ...
                    <el-form-item label="商品SKU">
                        <el-table :data="form.productSkuList" border style="width: 100%">
                            ...
                            <el-table-column label="图片" #default="scope" width="80">
                                <el-upload class="avatar-uploader" :action="imgUpload.url" :headers="imgUpload.headers" :show-file-list="false" :on-success="(response, uploadFile, fileList) =>handleSkuSuccess(response, uploadFile, fileList, scope.row)">
                ...
                <div v-if="activeIndex == 2">
                    <el-form-item label="详情图片">
                        <el-upload v-model:file-list="detailsImageTempUrlList" :action="imgUpload.url" :headers="imgUpload.headers" list-type="picture-card" multiple :on-success="handleDetailsSuccess" :on-remove="handleDetailsRemove">
                            <el-icon><Plus /></el-icon>
                        </el-upload>
                    </el-form-item>
                </div>
